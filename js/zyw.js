var version={
    ...
    ver:"2.1.1",
    appv:584,
    requireId:"https://gitcode.net/qq_32394351/dr/-/raw/master/js/zyw.js",
...
function xml2json(xmlText){//将xml结果转为json的obj对象
    ...
    if(!fetch(lib_path)){
        writeFile(lib_path, fetch('https://gitcode.net/qq_32394351/dr/-/raw/master/html/xml2json.html'));
    }
    if(!xmlText){
        return {}
    }
    ...
    function renderList(showList){//渲染列表
        ...
        for (let j in showList) {
            ...
            if(!是否支持魔断(id)){ // 为了兼容聚合解析
                ...
                blockRules = ['baidu.*.png', '.gif', '.png', '.ico', '.svg'];
            }
            allList.push({
                title: name,
