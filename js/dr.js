...
2022/11/06 推送优化; 
2022/11/05 推送优化; 
2022/11/03 增加tvbox推送功能; 
2022/11/02 二级增加嗅探缓存变量 嗅探缓存 = true; 请注意前面不需要var，否则DR规则嗅探缓存不生效
2022/11/01 取消blockRules
2022/08/24 废除部分代码
    ...
    if(parseInt(page)===1) {
        ...
        let settings = {
            ...
            url: $().rule((color,htmlTag,blank) => {
                ...
                d.push({
                    title:"TVB:"+color(lsg.getItem("推送","开"),"#ff7000"),
                    col_type:"text_3",
                    url:$('#noLoading#').lazyRule(()=>{
                        require(getVar('dr依赖'));
                        if(lsg.getItem("推送","开")==="开"){
                            lsg.setItem("推送","关");
                        }else{
                            lsg.setItem("推送","开");
                        }
                        refreshPage(false);
                        return "toast://已设置二级TVB推送组件显示为:"+lsg.getItem("推送");
                    })
                });
                d.push({
                    title:'清空翻页足迹',
...
function 嗅探(){ // 视界video://
    require(config.嗅探||version.jsRoot+'video.js');
}
...
function 二级推送组件(d,obj){ // {}
    let push_flag = lsg.getItem("推送","开");
    obj = obj || {line:'both'}; // top,bottom,none
    let tv_col = obj.col_type||'icon_small_3';
    log('准备渲染二级推送组件,启用推送:'+push_flag);
    if(push_flag==='开') {
                    ...
                    return $('如果本地已存在,将被覆盖,确定同步道长辅助嗅探规则?').confirm((sniffer_file)=>{
                        try {
                            ...
                            if(sniffer_json.length > 10){
                                let cnt = JSON.parse(sniffer_json).length;
                                writeFile(sniffer_file, sniffer_json);
                                return 'toast://同步成功,共计'+cnt+'条辅助嗅探规则'
                            }else{
                                return 'toast://同步失败'
                            }
                        } catch (e) {
                            return 'toast://同步出错:'+e.message
                        }
                    },sniffer_file);
                }else if(/编辑/.test(input)){
                    return "openFile://" + sniffer_file;
        ...
        d.push({
            ...
            url: $('#noLoading#').lazyRule(() => {
                ...
                let poster;
                try {
                    poster = findItemsByCls("vod_poster")[0];
                }catch (e) {
                    poster = {pic:''};
                }
                let content;
                try {
                    content = findItemsByCls("vod_content")[0];
                }catch (e) {
                    content = {desc:'暂无详情'};
                }
                let text1;
                ...
                // log(poster);
                try{
                    text1 = poster.title.split('<small>')[1].split('</small>')[0];
                }catch (e) {
                    text1 = '';
                }
                try{
                    text2 = poster.desc.split('<small>')[1].split('</small>')[0];
                }catch (e) {
                    text2 = '';
                }
                try{
                    play_from = findItemsByCls("vod_play_from").find(x => x.desc).desc;
                }catch (e) {
                    play_from = '';
                }
                try {
                    // let play_url = findItemsByCls("playList").map(x => x.title + '$' + x.url.split('@')[0]).join('#');
                    play_url = findItemsByCls("playList").map(x => x.title + '$' + x.url.replace(/\&/g, '＆＆').split('@')[0].trim()).join('#');
                }catch (e) {
                    play_url = '';
                }
                let vod = {
                    vod_title: text1.split('\n')[0],
                    type_name: text1.split('\n').length > 1 ? text1.split('\n')[1] :'',
                    vod_remarks: text2,
                    vod_pic: poster.pic,
                    vod_content: '【道长DR】'+content.desc,
                ...
                let sniffer;
                try {
                    sniffer = JSON.parse(fetch(sniffer_file));
                }catch (e) {
                    sniffer = [];
                }
                if(Array.isArray(sniffer) && sniffer.length > 0){
                    let now_sniffer = sniffer.find(it=>play_url.split('#')[0].split('$')[1].includes(it.host));
                    if(now_sniffer){
                        Object.assign(xyq_detail,now_sniffer);
                    }
                }
                log(xyq_detail);
                let state = post(tvb_info.now + 'action', {
                    timeout: 2000,
                    body: {do: 'push', url: JSON.stringify(xyq_detail)},
...
function 二级(parse,lazy,dp,错误){
    ...
    */
    ...
    嗅探缓存=(typeof(嗅探缓存)==='undefined'||!嗅探缓存)?false:嗅探缓存;
    ...
    if(样式==='滚动'){
        ...
        for(let i in tabs){
            ...
            d.push({
                ...
                col_type:"scroll_button",
                desc:nowTab===parseInt(i)?tab_name:'',
                extra:{
                    cls:'vod_play_from'
                }
            });
