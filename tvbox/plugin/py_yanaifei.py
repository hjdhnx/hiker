...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def homeVideoContent(self):
		rsp = self.fetch("https://yanetflix.com",headers=self.header)
		root = self.html(rsp.text)
		aList = root.xpath("//div[@class='module-poster-item module-item']")
		videos = []
		for a in aList:
	...
	def categoryContent(self,tid,pg,filter,extend):
		...
		url = 'https://yanetflix.com/vodshow/{0}.html'.format(params)
